...
public class CarrinhoUI extends javax.swing.JFrame {
    ...
    private void inserirVideos(DefaultTableModel modeloTabela) {
        for (int i = 0; i < listaVideos.videos.size(); i++) {
            ...
            if (listaVideos.videos.get(i).getTipo() == "filmagem") {
                Filmagem video = (Filmagem) listaVideos.videos.get(i);
                String[] dados = {"Vídeo",
                    "Tipo: " + video.getTipo() + "/ Duração: " + video.getDuracao() + " segundos",
                    String.format("%.1f", video.calcularTempoEstimadoDeServico()) + " horas",
                    "R$" + String.format("%.2f", video.calcularPrecoFinal())
                };
                modeloTabela.addRow(dados);

            } else if (listaVideos.videos.get(i).getTipo() == "animação") {
                Animacao video = (Animacao) listaVideos.videos.get(i);
                String[] dados = {"Vídeo",
                    "Tipo: " + video.getTipo() + "/ Duração: " + video.getDuracao() + " segundos",
                    String.format("%.1f", video.calcularTempoEstimadoDeServico()) + " horas",
                    "R$" + String.format("%.2f", video.calcularPrecoFinal())
        ...
        for (int i = 0; i < listaSites.sites.size(); i++) {
            if (listaSites.sites.get(i).getTipo() == "loja") {
                ...
                String[] dados = {"Site",
                    ...
                    "R$" + String.format("%.2f", site.calcularPrecoFinal())
                };
                modeloTabela.addRow(dados);
